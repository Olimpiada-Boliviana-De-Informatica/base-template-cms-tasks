...
\
  -l, --source-line=attach
\tPrints the corresponding source line (from the test generation data file)
\t inline after each test case.
\
  -L, --source-line=precede
\tPrints the corresponding source line (from the test generation data file)
\t before each test case on a separate line.
\
      --source-line=none
\tThe corresponding source line (from the test generation data file) is not printed for each test case.
...
function handle_option {
	...
	case "${curr_arg}" in
		...
		-d|--gen-data=*)
			...
			;;
		-l)
			SOURCE_LINE_PRINT_MODE="attach"
			;;
		-L)
			SOURCE_LINE_PRINT_MODE="precede"
			;;
		--source-line=*)
			fetch_nonempty_arg_value "SOURCE_LINE_PRINT_MODE" "-@" "--source-line" "source line printing mode"
			is_in "${SOURCE_LINE_PRINT_MODE}" "attach" "precede" "none" ||
				invalid_arg_with_usage "${curr_arg}" "undefined source line printing mode"
			;;
		--tests-dir=*)
...
[ -z "${flag_for_not_recreating_tests_dir}" -o -d "${tests_dir}" ] ||
	error_exit 3 "Error: tests directory '${tests_dir}' does not exist (needed due to flag '${flag_for_not_recreating_tests_dir}')."


command_exists dos2unix || cecho yellow "WARNING: dos2unix is not available. Line endings might be incorrect."

export SENSITIVE_RUN WARNING_SENSITIVE_RUN \
	UPDATE_MODE \
	SPECIFIC_TESTS SPECIFIED_TESTS_PATTERN \
	SOURCE_LINE_PRINT_MODE \
	SKIP_GEN SKIP_SOL SKIP_VAL
